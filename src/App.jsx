...
function ProductView({ product, quantity, setQuantity, liked, setLiked, goHome }) {
  ...
  return <main className="detail-page"><button className="back-link" onClick={goHome}><ArrowLeft size={16} /> Back to the current drop</button><div className="detail-grid"><div className="gallery"><div className="main-product-image"><img src={product.thumbs[activeImage]} alt={product.name} /><span className="gallery-count">0{activeImage + 1} / 03</span></div><div className="thumb-row">{product.thumbs.map((image, index) => <button className={activeImage === index ? 'thumb active' : 'thumb'} key={image} onClick={() => setActiveImage(index)}><img src={image} alt={`${product.name} view ${index + 1}`} /></button>)}</div></div><div className="detail-copy"><div className="eyebrow">{product.category} / {product.badge}</div><div className="detail-title-row"><h1>{product.name}</h1><button className={liked ? 'like-btn liked' : 'like-btn'} onClick={() => setLiked(!liked)} aria-label="Save product"><Heart size={20} fill={liked ? 'currentColor' : 'none'} /></button></div><div className="detail-status"><span className="status-dot" /> {product.status} · stock confirmed in WhatsApp</div><p className="detail-tagline">{product.tagline}</p><p className="detail-description">{product.description}</p><ul className="feature-list">{product.features.map((feature) => <li key={feature}><BadgeCheck size={17} /> {feature}</li>)}</ul><div className="price-block"><div><strong>{money(product.price)}</strong><small> per item · pay on delivery</small></div><span>Price in ZAR</span></div><div className="order-box"><div className="quantity"><button onClick={() => setQuantity(Math.max(1, quantity - 1))}><Minus size={15} /></button><b>{quantity}</b><button onClick={() => setQuantity(quantity + 1)}><Plus size={15} /></button></div><a className="order-button" href={whatsappUrl(product, quantity)} target="_blank" rel="noreferrer"><MessageCircle size={19} /> Check availability <ArrowUpRight size={17} /></a></div><div className="delivery-note"><Truck size={18} /><div><b>Delivery around {STORE_AREAS}</b><span>{DELIVERY_NOTE}</span></div></div><div className="accordion"><details open><summary>What happens after I tap? <ChevronDown size={17} /></summary><p>WhatsApp opens with this product and quantity filled in. We’ll reply personally to confirm stock, your address, delivery fee and timing. Nothing is dispatched without your confirmation.</p></details><details><summary>Can I ask for another colour or option? <ChevronDown size={17} /></summary><p>Yes. Send us a message before ordering and we’ll tell you what is currently available from our supplier.</p></details></div></div></div><div className="mobile-sticky-cta"><a href={whatsappUrl(product, quantity)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> Check availability · {money(product.price * quantity)}</a></div></main>;
}
